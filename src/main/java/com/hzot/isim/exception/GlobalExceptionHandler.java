package com.hzot.isim.exception;


import com.alibaba.fastjson.JSONObject;
import com.hzot.isim.util.RestResponse;
import com.hzot.isim.util.ToolUtil;
    ...
    public ModelAndView handleHttpMessageNotReadableException(HttpServletRequest request,
        ...
        if (ToolUtil.isAjax(request)) {
            try {
                ...
                if(e.getCause().getLocalizedMessage().contains("Data truncation")){
                    msg = "字段数据过长:"+e.getCause().getMessage();
                }else if(DataIntegrityViolationException.class.equals(e.getClass())){
                    if(e.getCause().getMessage().contains("Cannot delete or update a parent row: a foreign key constraint fails")){
                        msg = "该数据被其它数据引用，请删除或解除引用";
                    }else{
                        msg = "插入或更新数据错误："+e.getCause().getMessage();
                    }
                }
                response.setContentType("application/json;charset=UTF-8");
                PrintWriter writer = response.getWriter();
                RestResponse failResponse = RestResponse.failure(msg);
